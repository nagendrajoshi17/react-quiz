...
export default function QuizScreen() {
  ...
  const quizDataQuestion = quizdata.filter(question => question.category === 'World Knowledge');
  const quizDataAnswer = quizdata.filter(options => options.category === 'World Knowledge');
        ...
        <>
          ...
          <div className='answer'>
            {quizDataAnswer[currentQuestion].options.map((ans, i) => {
              const buttonStyle = {
                backgroundColor: selectedAnswer === null ? "" : ans.isCorrect ? "green" : selectedAnswer === ans.isCorrect ? "red" : "",
              };
              return (
                <button
